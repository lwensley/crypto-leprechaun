function makeResponsive() {
 ...
 // const left_margin = 50;
  ...
  const candle_win_height = (window.innerHeight - margin.bottom - margin.top) * 4/5;
  const candle_win_width = window.innerWidth - margin.left - margin.right; //size of the candlestick and volume rect 
  const indicator_win_height = (window.innerHeight - margin.bottom - margin.top) * 1/5;; //size of indicator rect 
...
function updateToolTip(chosenIaxis, iLinesGroup) {
    ...
    else if (cchosenIaxis === bitcoin_dominance){
  ...
  // function used for updating indicator scale var upon click on axis label
  function iScale(data, chosenIaxis) {
    // create scales
  ...
  function renderIaxis(newIScale, iAxis) {
    var leftAxis = d3.axisLeft(newIScale);
  ...
  // function used for updating lines group with a transition to
  // new lines
  function renderLines(iLinesGroup, newIScale, chosenIaxis) {

    iLinesGroup.transition()
      .duration(1000)
...
//   console.log(indicator_2);
  ...
  var svg = d3.select("#svg-area") //1000 width and 500 heihgt good on my laptop
    ...
    .attr("width", svgWidth); // ; only for the last one, no , either
...
//   const i_scaler =(indicator_win_height)/(max_i1-min_i1); //scaler for indicator 1
...
//   .ticks(6);
  ...
  svg.append("g")
  .attr("transform", `translate(${margin.left}, ${margin.topn})`) // main price axis 
...
// // append initial line
// var iLineGroup = svg.selectAll("line")
// .data(Data)
// .enter()
// .append("circle")
...
var IndicatorGroup = updateToolTip(chosenIaxis, IndicatorGroup);
...
ilabelsGroup.selectAll("text")
  ...
  if (valuei !== chosenIaxis) {
    ...
    IndicatorGroup = renderLines(iLinesGroup, iLinearScale, chosenIaxis);
    ...
    IndicatorGroup = updateToolTip(chosenIaxis, IndicatorGroup);
